...
func MessengerTools(s *server.MCPServer, token *string) {
	doorayMessengerTool := mcp.NewTool("dooray_messenger",
		...
		mcp.WithString("operation",
			mcp.Required(),
			mcp.Description("The operation to perform. 'send': direct message to a member (requires to, message). 'send_channel': send message to a channel (requires channelId, message). 'find_channels': list channels the current user belongs to. 'find_channel_logs': fetch recent messages of a channel (requires channelId)"),
			mcp.Enum("send", "send_channel", "find_channels", "find_channel_logs"),
		),
		mcp.WithString("to",
		...
		mcp.WithString("channelId",
			mcp.Description("channel id (required for 'send_channel' and 'find_channel_logs')"),
		),
